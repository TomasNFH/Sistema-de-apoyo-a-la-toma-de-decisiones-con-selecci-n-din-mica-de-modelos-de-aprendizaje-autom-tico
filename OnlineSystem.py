...
with gr.Blocks() as EDA_TAB:
    print('\nInitialize -> gr.BLOCK EDAAAAA \n')
    print('EDA_TAB_FLAG second tab')
    print(EDA_TAB_FLAG)

    
    aux_RFLAG = gr.State(0)


    gr.Markdown("## Here is the EDA (of the dataset)")
    aux_button = gr.Button('GO')
    aux_button.click(lambda count: count + 1, aux_RFLAG, aux_RFLAG, scroll_to_output=True)

    @gr.render(inputs=[aux_RFLAG])
    def button_aux(AUXXX):
        global DATAcasted
        if AUXXX>0:
            gr.Markdown("## Here is the EDA (of the dataset)sdasdsadasdasda")
            print('EDA_TAB_FLAG button')
            print(EDA_TAB_FLAG)
...
######### ALL #########
demo = gr.TabbedInterface([upload_fileTAB, EDA_TAB, bye_world], ["Data acquisition", "EDA", "aux"])
